...
const { sendBulletinPostEmail } = require('../services/emailService');
...
exports.createPost = async (req, res) => {
  try {
    ...
    if (!organization) {
      console.error('Organization not found for bulletin post:', req.user.organization);
    }

    // Get all active users in the same organization for email notification
    ...
    if (users.length > 0 && organization) {
      try {
        const emailResult = await sendBulletinPostEmail({
          organization,
          users,
          admin: req.user,
          post
        });
        console.log('Bulletin post email notifications sent successfully:', emailResult);
      } catch (emailError) {
        console.error('Failed to send bulletin post email notifications:', emailError);
        // Don't fail the request if email fails
      }
    }

    // Notify all users (including custom admins) - existing notification
