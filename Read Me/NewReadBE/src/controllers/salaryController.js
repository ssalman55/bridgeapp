...
exports.getAllStructures = async (req, res) => {
  try {
    ...
    const structuresWithSignedUrls = structures.map(structure => {
      const staff = structure.staff;
      if (staff && staff.profileImage && staff.profileImage.startsWith('profile-images/')) {
        try {
          staff.profileImage = getSignedUrl(staff.profileImage, 3600);
        } catch (error) {
    ...
    res.json(structuresWithSignedUrls);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.downloadSalaryCSVTemplate = async (req, res) => {
  try {
    // Get all staff for the admin's organization (regardless of role)
    const staffList = await User.find({ organization: req.user.organization, status: { $ne: 'archived' } })
      .select('_id fullName')
      .sort({ fullName: 1 });
...
// Flush variable salary fields (bonus, reimbursements, deductions, notes)
exports.flushVariableFields = async (req, res) => {
  try {
    // Find all salary structures that are in draft status and not locked
    ...
    if (structures.length === 0) {
      return res.json({
        success: true,
        message: 'No eligible salary structures found for flushing.',
        recordsUpdated: 0,
        timestamp: new Date()
      });
    }

    // Update all eligible structures with recalculated netSalary
    const updateResult = await SalaryStructure.updateMany(
      {
        organization: req.user.organization,
        status: 'draft',
        locked: false
      ...
      [
        {
          $set: {
            ...
            notes: '',
            updatedAt: new Date(),
            // Recalculate netSalary: basic + housing + utility + bonus + transport + reimbursements - (deductions + taxes)
            netSalary: {
              $add: [
                '$basic',
                '$housing',
                ...
                0, // bonus (now 0)
                '$transport',
                0, // reimbursements (now 0)
                {
                  $multiply: [
                    { $add: [0, '$taxes'] }, // deductions (now 0) + taxes
                    -1
                  ]
                }
              ]
    ...
    await PayrollAuditLog.create({
      date: new Date(),
      action: 'flush_salary_variables',
      performedBy: req.user._id,
      prevValue: `${structures.length} structures with variable fields`,
      newValue: 'All variable fields reset to 0',
      notes: `Flushed variable salary fields for ${updateResult.modifiedCount} staff members`,
      organization: req.user.organization
    });

    res.json({
      success: true,
      message: `Variable salary fields flushed successfully. ${updateResult.modifiedCount} records updated.`,
